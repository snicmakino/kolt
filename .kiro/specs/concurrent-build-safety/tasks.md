...
- Pre-existing TOCTOU race in `ensureDirectoryRecursive` (two procs racing to create the same `~/.kolt/cache/<g>/<a>/<v>/` tree) was **outside** this spec's boundary; scenario 4 originally sidestepped it with a serial warm-up. Resolved in #263: `mkdir` now tolerates `errno == EEXIST` and the warm-up phase is gone, so scenario 4 races a fully cold cache.
...
- Validation follow-up: `doCheck` was missing from the original 3.1 wrap list but writes `kolt.lock` via `resolveDependencies`. Wrapped with `withProjectLock` and refactored to delegate to `doCheckInner`, which calls `doBuildInner` for native targets to avoid the same OFD-fresh re-acquire pattern as `doTest → doBuildInner`. ADR 0029 §1 and design.md §Architecture / §File Structure Plan / §Implementation Notes updated to include `doCheck` in the locked set.
